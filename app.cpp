...
#define IO_SAMPLE_ALIAS_POSITION 1, 0
#define IO_SAMPLE_VENDORID_PRODUCTCODE 0x00300077, 0x00000001
...
const static ec_pdo_entry_reg_t pdo_entry_regs[] = {
    ...
    {IO_SAMPLE_ALIAS_POSITION, IO_SAMPLE_VENDORID_PRODUCTCODE, 0x7000, 0x01,
     &user_data.output_bit1, &user_data.offset_output_bit1},
    {IO_SAMPLE_ALIAS_POSITION, IO_SAMPLE_VENDORID_PRODUCTCODE, 0x7000, 0x02,
     &user_data.output_bit2, &user_data.offset_output_bit2},
    {IO_SAMPLE_ALIAS_POSITION, IO_SAMPLE_VENDORID_PRODUCTCODE, 0x6000, 0x01,
     &user_data.input_bit1, &user_data.offset_input_bit1},
    {IO_SAMPLE_ALIAS_POSITION, IO_SAMPLE_VENDORID_PRODUCTCODE, 0x6000, 0x02,
...
void App::Config() {
  std::vector<EthercatNode::SalveParam>
      salve_param;  //用于存储所有从站的配置信息
  EthercatNode::SalveParam sample;
  // 这里从readme中的信息获取一下从站的各种信息并配置从站
  // sample.alias = 0; //别名
  // sample.position = 0;  //位置
  // sample.vendor_id = 0x5a65726f;  //供应商
  // sample.product_code = 0x29252;
  // salve_param.push_back(sample);  //将从站信息放入salve_param
  sample.alias = 1;
  sample.position = 0;
  sample.vendor_id = 0x00300077;
  sample.product_code = 0x00000001;
  salve_param.push_back(sample);
  // 这里只有一个master，所以index是0
  Initialize(salve_param, pdo_entry_regs,
             0);  //初始化将上面准备好的从站列表 salve_param 传入
                  // Initialize,pdo_entry_regs：可能是事先定义好的 PDO（Process
                  // Data Object）映射表
}
...
bool App::InitializeDevices() {
  ...
  while (*running_ && (!ret)) {
    ...
    // 如果从站 OP，直接返回 true
    if (slave_configs_.at(0).sc_state.operational == 1) {
      KAYLORDUT_LOG_INFO("Slave is OP, ready for LED test");
      ret = true;
    }
    // auto status = EC_READ_U16(process_data_ + user_data.status_word);
    // auto position = EC_READ_U32(process_data_ + user_data.position_value);
